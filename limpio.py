...
OUTPUT_EXCEL = "Resultados_Anonimizador_Hibrido_v4.xlsx"
...
def postprocess_person_entity(
    ...
    """
    Limpia, valida y clasifica entidades de persona.
    Devuelve (start, end, entidad) o None si se descarta.
    """
    start, end = trim_person_name_span(texto, start, end)
    if end <= start:
        return None

    span = texto[start:end]
    if not is_probable_person_name(span):
        return None

    ent_canon = classify_person_role_by_context(
        texto, start, end, default=ent_canon
    )

    return start, end, ent_canon
...
def detect_entities_ner(id_texto: int, texto: str, ner_pipeline) -> List[Prediction]:
    ...
    for ent in ner_pipeline(texto):
        start, end = int(ent["start"]), int(ent["end"])
        score = float(ent.get("score", 1.0))
        raw_label = ent.get("entity_group") or ent.get("entity", "")

        ent_canon = canonicalize_entity_name(
            map_ner_label_to_entity(raw_label)
        )

        if score < NER_SCORE_BY_ENTITY.get(ent_canon, NER_SCORE_THRESHOLD):
            continue

        span_raw = texto[start:end]

        # EDAD: exige marcador explícito
        if ent_canon == "EDAD" and not re.search(r"(?:años?|a\.?)\b", span_raw.lower()):
            continue

        # PERSONAS: limpieza + validación + rol
        if ent_canon in {"NOMBRE_PACIENTE", "NOMBRE_PROFESIONAL"}:
            out = postprocess_person_entity(texto, start, end, ent_canon)
            if out is None:
                continue
            start, end, ent_canon = out

        # FECHAS: especializar por contexto
        ...
        # Normalización final del span
        start, end = normalize_span(texto, start, end, ent_canon)
        start, end = expand_span_left_titles(texto, start, end, ent_canon)
        if end <= start:
            continue
    ...
    # Fallback nombre inicial ("Roberto Sánchez, ...")
    if not any(p.entity in {"NOMBRE_PACIENTE", "NOMBRE_PROFESIONAL"} for p in preds):
        m = re.search(
        ...
        if m:
            ...
            if is_probable_person_name(texto[s:e]):
                ent_canon = classify_person_role_by_context(
                    texto, s, e, default="NOMBRE_PACIENTE"
                )
                s, e = normalize_span(texto, s, e, ent_canon)
                preds.append(
                    Prediction(
                        id_texto=id_texto,
                        start=s,
